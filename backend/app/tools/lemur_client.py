...
async def run_lemur_prompt_text(transcript_text: str, prompt: str) -> str:
    """
    Sends raw transcript text directly to Groq — no AssemblyAI fetch.
    Used by tools that already have the text from Supabase.
    """
    headers = {
        "authorization": f"Bearer {settings.GROQ_API_KEY}",
        "content-type": "application/json",
