...
// const TOOL_VOICE = {
//   get_transcript:    'Alright, pulling up the full transcript.',
//   summarize_meeting: 'Let me put together a summary of the meeting.',
//   action_items:      'Got it, here are the action items.',
// }

// const TOOL_DONE = {
//   get_transcript:    'Done! You can scroll through or download it below.',
//   summarize_meeting: 'Done! Here is what the meeting covered.',
//   action_items:      'Done! Those are all the action items from the meeting.',
// }
...
// // ── Typewriter with Stable Completion Guard ──────────────────────────────────
...
//       varying float vY; varying float vFresnel; uniform float uAmp;
//       void main() {
...
//       uSloshX: { value: 0 }, uSloshY: { value: 0 },
//     }
...
//     hGeo.setAttribute('normal',   new THREE.BufferAttribute(hNorms, 3))
//     const hUniforms = {
//       uTime: { value: 0 }, uAmp: { value: 0 },
//       uSloshX: { value: 0 }, uSloshY: { value: 0 },
//     }
...
//         .replace(
//           '(1.0 + rimBoost * 5.0) * (1.0 + uAmp * 1.2 + sloshMag * 0.4)',
//           '(2.0 + rimBoost * 7.0) * (1.0 + uAmp * 1.4 + sloshMag * 0.5)'
//         )
//         .replace(
//           '(0.10 + uAmp * 0.35 + sloshMag * 0.12)',
//           '(0.06 + uAmp * 0.20 + sloshMag * 0.08)'
//         ),
//       fragmentShader: dark ? `
//         varying float vY; varying float vFresnel; uniform float uAmp;
//         void main() {
//           vec2 uv = gl_PointCoord - 0.5;
//           if (length(uv) > 0.5) discard;
//           float soft = 1.0 - smoothstep(0.1, 0.5, length(uv));
//           float t = clamp((vY + 1.0) * 0.5, 0.0, 1.0);
//           vec3 col = t > 0.5
//             ? mix(vec3(0.90,0.10,1.00), vec3(0.20,1.00,1.00), (t-0.5)*2.0)
//             : mix(vec3(1.00,0.50,0.00), vec3(0.90,0.10,1.00), t*2.0);
//           col = col * (1.5 + uAmp * 1.5);
//           gl_FragColor = vec4(col, soft * pow(vFresnel, 0.6) * (0.8 + uAmp * 0.5));
//         }
//       ` : `
//         varying float vY; varying float vFresnel; uniform float uAmp;
//         void main() {
//           vec2 uv = gl_PointCoord - 0.5;
//           if (length(uv) > 0.5) discard;
//           float soft = 1.0 - smoothstep(0.05, 0.5, length(uv));
//           float t = clamp((vY + 1.0) * 0.5, 0.0, 1.0);
//           vec3 col = t > 0.5
//             ? mix(vec3(0.55,0.10,0.85), vec3(0.00,0.72,0.80), (t-0.5)*2.0)
//             : mix(vec3(0.90,0.10,0.65), vec3(0.55,0.10,0.85), t*2.0);
//           col = col * (0.5 + uAmp * 0.5);
//           gl_FragColor = vec4(col, soft * pow(vFresnel, 0.5) * (0.6 + uAmp * 0.4));
//         }
//       `,
//       transparent: true,
//       blending: dark ? THREE.AdditiveBlending : THREE.NormalBlending,
...
// function TranscriptPanel({ content, dark, onDone }) {
//   const { displayed, done } = useTypewriter(content, !!content, onDone)
...
//   return (
//     <div style={{ height: '100%', display: 'flex', flexDirection: 'column', padding: '28px 24px 20px' }}>
//       <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: tc2, marginBottom: 16 }}>
//         Transcript
//       </p>
...
// function SummaryPanel({ content, dark, onDone }) {
//   const flat = content ? [
...
//   const { displayed } = useTypewriter(fullText, flat.length > 0, onDone)
...
//   return (
//     <div style={{ height: '100%', display: 'flex', flexDirection: 'column', padding: '28px 24px 20px' }}>
//       <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: tc2, marginBottom: 20 }}>
//         Meeting Summary
//       </p>
//       <div style={{ flex: 1, overflowY: 'auto', paddingRight: 4 }} className="panel-scroll">
...
//               <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: ac, marginBottom: 10 }}>
//                 {section}
//               </p>
//               {items.map((item, i) => (
...
//       <div style={{ flex: 1, overflowY: 'auto', paddingRight: 4 }} className="panel-scroll">
//         {items.length === 0 ? (
//           <p style={{ fontSize: 14, color: tc2, textAlign: 'center', marginTop: 40 }}>
//             No clear action items or tasks detected in this meeting.
//           </p>
//         ) : (
...
//   // Original active axis offset
//   const activeAxis = panelOpen ? 'calc(50% + 200px)' : '50%'
...
//         const force  = -STIFFNESS * (val - target)
...
//   // ── speakText with smooth single-buffer playback ────────────────────────
...
//     try {
//       await speakText(TOOL_VOICE[tool] || 'Working on that...')
...
//       // Done speech triggered exclusively once when typewriter finishes writing
//     } catch (err) {
//       console.error('[Tool execution failed]:', err)
//       setToolData(prev => ({ ...prev, [tool]: { status: 'error', content: null } }))
//       await speakText("I ran into an issue retrieving that. Please try again.")
//     } finally {
...
//     panelShadow: '0 8px 40px rgba(0,0,0,0.4)',
...
//     panelBorder: 'rgba(26,115,232,0.18)',
//     panelShadow: '0 4px 32px rgba(26,115,232,0.10), 0 2px 8px rgba(0,0,0,0.06)',
...
//   const phaseLabel  = {
//     idle: 'Hold to speak', listening: 'Listening...',
//     thinking: 'Thinking...', speaking: 'Nio',
//   }[phase] || 'Hold to speak'

//   const accentColor   = dark ? 'rgba(99,102,241,0.9)' : '#6366f1'
//   const SPHERE_SIZE   = 300
//   const CTRL_BOTTOM   = 52
//   const MIC_SIZE      = 72
...
//         background: th.bgGlow,
//         opacity: 0.4, pointerEvents: 'none', filter: 'blur(70px)',
//         left: activeAxis,
//         top: '50%',
...
//       {/* Top bar — Original fixed center wordmark */}
//       <div style={{
...
//       }}>
//         <span style={{
//           fontSize: 16, fontWeight: 600, color: th.wordmark, letterSpacing: '0.08em',
//         }}>
//           meetcore
//         </span>
//         <span style={{
...
//       {/* LEFT — Tool Panel (Original 400px width & 64px top positioning) */}
//       <div style={{
//         position: 'absolute',
//         top: 64, bottom: 40,
//         left: panelOpen ? 60 : -420,
//         width: 400,
...
//         zIndex: 5,
//       }}>
//         {activeTool === 'get_transcript'    && toolData.get_transcript.content    && <TranscriptPanel   content={toolData.get_transcript.content}    dark={dark} onDone={() => speakText(TOOL_DONE.get_transcript)} />}
//         {activeTool === 'summarize_meeting' && toolData.summarize_meeting.content  && <SummaryPanel      content={toolData.summarize_meeting.content}  dark={dark} onDone={() => speakText(TOOL_DONE.summarize_meeting)} />}
//         {activeTool === 'action_items'      && toolData.action_items.content       && <ActionItemsPanel  content={toolData.action_items.content}       dark={dark} onDone={() => speakText(TOOL_DONE.action_items)} />}
//         {activeTool && !toolData[activeTool]?.content && (
//           <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
//             <p style={{ fontSize: 13, color: dark ? 'rgba(255,255,255,0.3)' : '#9aa0a6' }}>
...
//       {/* Caption — Fixed height, wider container, scrollable with hidden scrollbar */}
//       {(caption || error) && (
//         <div
...
//             <path
//               d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.9 9.5a19.79 19.79 0 01-3.07-8.68A2 2 0 012.81 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.91 9.91a16 16 0 006.18 6.18l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"
//               fill="#ff4444"
//             />
...
function FluidSphere({ canvasRef, amplitudeRef, sloshRef, dark }) {
  useEffect(() => {
    ...
    const haloMat = new THREE.ShaderMaterial({
      uniforms: hUniforms, vertexShader,
      fragmentShader: dark ? fragmentShaderDark : fragmentShaderLight,
      transparent: true,
      blending: dark ? THREE.AdditiveBlending : THREE.NormalBlending,
      depthWrite: false,
...
function TranscriptPanel({ content, dark }) {
  const { displayed, done } = useTypewriter(content, !!content, null)
  const tc  = dark ? 'rgba(255,255,255,0.82)' : '#1a1a2e'
  const tc2 = dark ? 'rgba(255,255,255,0.4)'  : '#9aa0a6'
...
function SummaryPanel({ content, dark }) {
  const flat = content ? [
    ...(content.key_points     || []).map(t => ({ section: 'Key Points',    text: t })),
    ...(content.decisions      || []).map(t => ({ section: 'Decisions',      text: t })),
    ...(content.open_questions || []).map(t => ({ section: 'Open Questions', text: t })),
  ...
  const fullText = flat.map(i => i.text).join('\n')
  const { displayed } = useTypewriter(fullText, flat.length > 0, null)
...
function ActionItemsPanel({ content, dark }) {
  const [checked, setChecked] = useState({})
  const items    = Array.isArray(content) ? content : (content?.items || [])
  const fullText = items.map(i => i.description).join('\n')
  const { displayed } = useTypewriter(fullText, items.length > 0, null)
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', padding: '28px 24px 20px' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingRight: 4 }} className="panel-scroll">
        {items.length === 0 ? (
          <p style={{ fontSize: 14, color: tc2, textAlign: 'center', marginTop: 40 }}>
            No clear action items detected in this meeting.
          </p>
        ) : (
          visible.map(({ show, idx, owner, deadline, description }) => (
            <div key={idx} style={{ display: 'flex', gap: 12, marginBottom: 16, alignItems: 'flex-start' }}>
...
function DraftEmailPanel({ content, dark }) {
  const [copied, setCopied] = useState(false)
  const subject  = content?.subject || ''
  const body     = content?.body    || ''
  const fullText = `Subject: ${subject}\n\n${body}`
  const { displayed, done } = useTypewriter(fullText, !!fullText, null)
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingRight: 4 }} className="panel-scroll">
        ...
        <div style={{ padding: '10px 14px', borderRadius: 10, background: panelBg }}>
          ...
          <p style={{ fontSize: 13, lineHeight: 1.75, color: tc, margin: 0, whiteSpace: 'pre-wrap' }}>
            {displayed.slice(`Subject: ${subject}\n\n`.length)}
            {!done && <span style={{ opacity: 0.4 }}>▍</span>}
          </p>
        </div>
      </div>
  ...
  const runTool = useCallback(async (tool) => {
    ...
    try {
      ...
      const res = await fetch(`${API}${endpoint}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ meeting_id: meetingId, transcript_id: meetingId }),
      })
  ...
  const stopRecording = useCallback(async () => {
    ...
    recorder.onstop = async () => {
      ...
      try {
        ...
        if (detectedTool) {
          // Speak Nio's natural answer first if present
          ...
          // Fire background email silently if requested
        ...
        // ── Background email with no drawer tool ──────────────────────────────
        if (nioData.background_tool?.name === 'send_email') {
          const params = nioData.background_tool.params || {}
          fetch(`${API}/tools/send-email`, {
            method: 'POST',
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ fontSize: 16, fontWeight: 600, color: th.wordmark, letterSpacing: '0.08em' }}>
          meetcore
        </span>
        <span style={{
          position: 'absolute', right: 68,
      ...
      <div style={{
        ...
        {activeTool === 'get_transcript'    && toolData.get_transcript.content    && <TranscriptPanel  content={toolData.get_transcript.content}    dark={dark} />}
        {activeTool === 'summarize_meeting' && toolData.summarize_meeting.content  && <SummaryPanel     content={toolData.summarize_meeting.content}  dark={dark} />}
        {activeTool === 'action_items'      && toolData.action_items.content       && <ActionItemsPanel content={toolData.action_items.content}       dark={dark} />}
        {activeTool === 'draft_email'       && toolData.draft_email.content        && <DraftEmailPanel  content={toolData.draft_email.content}        dark={dark} />}
        {activeTool && !toolData[activeTool]?.content && (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <p style={{ fontSize: 13, color: dark ? 'rgba(255,255,255,0.3)' : '#9aa0a6' }}>
              {toolData[activeTool]?.status === 'error' ? 'Something went wrong.' : 'Working on it...'}
      ...
      <div style={{
        ...
        <button onClick={onEnd} style={{
          ...
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
            <path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.9 9.5a19.79 19.79 0 01-3.07-8.68A2 2 0 012.81 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.91 9.91a16 16 0 006.18 6.18l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"
              fill="#ff4444"/>
            <line x1="23" y1="1" x2="1" y2="23" stroke="#ff4444" strokeWidth="2.2" strokeLinecap="round"/>
          </svg>
        </button>
